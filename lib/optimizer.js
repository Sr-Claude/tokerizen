...
const CONFIG = {
  ...
  // CACHE_FILE='' (cadena vacía) desactiva la persistencia a disco.
  ...
  // Caché de respuestas (opt-in x-cache-response): TTL corto y cota de entradas.
  RESPONSE_CACHE_TTL_MS: intFromEnv('RESPONSE_CACHE_TTL_MS', 300_000),
  RESPONSE_CACHE_MAX: intFromEnv('RESPONSE_CACHE_MAX', 500),
  // Fracción del presupuesto (x-budget-usd) a la que se dispara el webhook de
  // aviso temprano (x-budget-webhook / BUDGET_ALERT_WEBHOOK_URL). Fuera de (0,1) -> 0.8.
  BUDGET_ALERT_THRESHOLD_PCT: (() => {
    const v = intFromEnv('BUDGET_ALERT_THRESHOLD_PCT', 0.8);
    return (Number.isFinite(v) && v > 0 && v < 1) ? v : 0.8;
  })(),
};

// ==================== UTILIDADES ====================
// null-safe: deepClone(undefined) -> undefined. structuredClone (Node 17+) es más
// rápido y usa menos memoria que el round-trip JSON en cuerpos grandes.
...
// Precio de entrada (USD por millón de tokens) según la familia del modelo,
// para estimar el ahorro real en dólares en vez de usar una tarifa fija.
function modelInputPricePerMTok(model) {
  const m = model || '';
  if (/fable|mythos/i.test(m)) return 10;
  if (/opus/i.test(m)) return 5;
  if (/haiku/i.test(m)) return 1;
  return 3; // sonnet y desconocidos
}

// Precio de salida (USD por millón de tokens) por familia.
function modelOutputPricePerMTok(model) {
  const m = model || '';
  if (/fable|mythos/i.test(m)) return 50;
  if (/opus/i.test(m)) return 25;
  if (/haiku/i.test(m)) return 5;
  return 15; // sonnet y desconocidos
}

// Coste estimado en USD de una respuesta a partir de su `usage` real.
// Tarifas de caché de Anthropic: lectura ≈0.1x entrada, escritura ≈1.25x entrada.
function estimateCostUSD(usage, model) {
  if (!usage) return 0;
  const pin = modelInputPricePerMTok(model);
  const pout = modelOutputPricePerMTok(model);
  return (
...
// JSON determinista (claves ordenadas recursivamente) para claves de caché:
// dos objetos con las mismas claves en distinto orden producen el mismo string.
...
// Clave de la caché de respuestas: hash de (ámbito de API key + petición normalizada).
// Se excluye lo que no afecta al contenido de la respuesta (stream, metadata).
// El ámbito evita que dos usuarios distintos compartan respuestas cacheadas.
function responseCacheKey(scope, body) {
  const c = { ...(body || {}) };
  delete c.stream;
  delete c.metadata;
  return crypto.createHash('sha256').update(`${scope}|${stableStringify(c)}`).digest('hex');
}

// ¿Es cacheable esta respuesta? Solo terminaciones limpias de texto: nunca
// tool_use a medias, refusals ni truncados por max_tokens.
function isResponseCacheable(response) {
  return !!response && (response.stop_reason === 'end_turn' || response.stop_reason === 'stop_sequence');
}

// ¿El cliente ya colocó sus propios breakpoints de caché? Si es así, el proxy
// NO los reubica: clientes como Claude Code ya optimizan su propia caché y
// moverles los breakpoints invalidaría su prefijo cacheado.
function hasClientCacheControl(system, messages, tools) {
...
function calculateMaxTokens(messages, tools, clientMax, model) {
  ...
  let suggested;
  // Solo tratamos como "respuesta corta" mensajes realmente cortos: un mensaje
  // largo que empiece por "no" ("no entiendo este código, explícamelo...") NO
  // debe truncarse a 30 tokens.
  const shortAffirmation = (/^(yes|no|ok|okay|sí|si)\b/.test(lower) || /\bconfirm\b/.test(lower)) && lower.length < 25;
  if (shortAffirmation || (lower.length < 10 && !lower.includes('\n'))) {
    suggested = 30;
...
function parseBatchResponse(text) {
  ...
  // Array disperso indexado por número de tarea: si el modelo omite la tarea N,
  // results[N-1] queda undefined. NO se compacta: compactar desplazaría las
  // respuestas siguientes y cada cliente recibiría la respuesta de otra tarea.
  return results;
}
